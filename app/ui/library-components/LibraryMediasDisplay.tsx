...
import CustomTooltip from '../ux-components/Tooltip' // [修改] 统一使用 CustomTooltip
       ...
       >
        {selectedDocIdsForDelete.includes(doc.id) ? (
         ...
         />
          ...
          // [汉化] & [颜色修复]
       <CustomTooltip title="元数据和下载" size="small">
        <IconButton onClick={() => setMediaToExplore(doc)} aria-label="Explore media" sx={{ px: 0.5 }}>
         <Info
          sx={{ ...CustomizedIconButton, color: palette.primary.main, fontSize: '1.3rem', '&:hover': { color: palette.primary.light } }}
         />
        </IconButton>
       </CustomTooltip>
      )
