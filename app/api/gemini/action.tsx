...
async function getGenerativeModel(): Promise<GenerativeModel> {
  ...
  if (!projectId || !location || !geminiModel) {
    throw new Error("缺少 Vertex AI 所需的环境变量 (项目ID, 区域, 或模型名称)。");
  }
...
export async function translatePromptToEnglish(
  ...
  let uid: string;
  try {
    const result = await verifyTokenAndCheckQuota("gemini", idToken);
    uid = result.uid;
  } catch (error: any) {
  ...
  try {
    ...
    const prompt = `Translate the following text to English. Only return the translated text, without any introductory phrases or quotation marks. Text to translate: "${textToTranslate}"`;
    ...
    if (!translatedText) {
      throw new Error("Gemini 未能返回有效的翻译结果。");
    }

    await deductQuota(uid, "gemini");
    return translatedText.trim();
  } catch (error: any) {
    console.error("在 translatePromptToEnglish 中出错:", error);
    return { error: "使用 Gemini 翻译 prompt 失败。" };
  }
}


export async function getDescriptionFromGemini(
  base64Image: string,
  type: string,
  idToken: string
): Promise<string | { error: string }> {
  ...
  try {
    ...
    console.error("getDescriptionFromGemini 的认证/配额检查失败:", error.message);
    return { error: error.message };
  }
  ...
  try {
    ...
    if (!candidate?.content?.parts?.[0]?.text) {
      throw new Error("来自 Gemini API 的响应结构无效。");
    }
    ...
    return {
      error: "从 Gemini 获取描述时出错。",
    };
  }
}

export async function getFullReferenceDescription(
  base64Image: string,
  type: string,
  idToken: string
): Promise<string | { error: string }> {
  ...
  try {
    ...
    console.error("getFullReferenceDescription 的认证/配额检查失败:", error.message);
    return { error: error.message };
  }
  ...
  try {
    ...
    if (!resp.response) {
      throw new Error("从 generateContent 调用中未找到响应对象。");
    }
...
export async function getPromptFromImageFromGemini(
  ...
  try {
    ...
    console.error("getPromptFromImageFromGemini 的认证/配额检查失败:", error.message);
    return { error: error.message };
  }
...
---
    ...
    console.error(JSON.stringify(await truncateLog(error), undefined, 4));
    return {
      error: "使用 Gemini 从图片获取 prompt 时出错。",
    };
  }
}

export async function getPromptFromVideoFromGemini(
  gcsUri: string,
  idToken: string
): Promise<string | { error: string }> {
  let uid: string;
  try {
    ...
    console.error("getPromptFromVideoFromGemini 的认证/配额检查失败:", error.message);
    return { error: error.message };
  }
...
Do not add any conversational text, preambles, or explanations. Your output should be only the structured prompt itself.`,
    ...
    console.error(JSON.stringify(await truncateLog(error), undefined, 4));
    return {
      error: "使用 Gemini 从视频获取 prompt 时出错。",
    };
  }
}
